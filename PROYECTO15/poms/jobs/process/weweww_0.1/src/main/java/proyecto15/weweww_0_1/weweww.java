 ...
 * @status
 */
public class weweww implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "weweww.log");
	}
	private static org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(weweww.class);
	...
	private final String jobName = "weweww";
	...
	JobStructureCatcherUtils talendJobLog = new JobStructureCatcherUtils(jobName, "_vJyokBUDEeqbh65aDacLVA", "0.1");
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(weweww.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_PROYECTO15_weweww = new byte[0];
		static byte[] commonByteArray_PROYECTO15_weweww = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_PROYECTO15_weweww.length) {
					if (length < 1024 && commonByteArray_PROYECTO15_weweww.length == 0) {
						commonByteArray_PROYECTO15_weweww = new byte[1024];
					} else {
						commonByteArray_PROYECTO15_weweww = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_PROYECTO15_weweww, 0, length);
				strReturn = new String(commonByteArray_PROYECTO15_weweww, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_PROYECTO15_weweww) {
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final weweww wewewwClass = new weweww();

		int exitCode = wewewwClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'weweww' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'weweww' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = weweww.class.getClassLoader()
					.getResourceAsStream("proyecto15/weweww_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = weweww.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : weweww");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 44041 characters generated by Talend Cloud Big Data on the 2 de diciembre de
 * 2019 14:01:50 CET
 ************************************************************************************************/